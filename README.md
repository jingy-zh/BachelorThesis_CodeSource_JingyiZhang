...
* **API Layer (`*_router.py`):** FastAPI routers handling HTTP requests for chat streaming, feedback, and document CRUD operations.
    ...
    * **feedback_router.py:** FastAPI endpoints for feedback management. The router of the function 'sending email to the dedicated person when the user flags the answer as outdated knowledge' is defined in the functions `def _send_email()` and `async def save_feedback ()`.**
...
* **Service Layer** The core functionality logic.
   ...
   * **sap_documents** Core functionality in sap document management, retrieval, and validity monitoring.
      ...
      * **document_processor.py:** Processing document using LangChain before retrieval, including extracting text, images, and tables. Text is segements into chunks, images and tables are stored in AWS S3 bucket.
        
      * **document_search.py:** _Managing document embeddings in the vector database, where RAG retrieves relevant fragments from database._ Hybrid retrieving: **(`def search ()`)**; Updating metadata fields without re-ingesting embeddings **(`def update_document_metadata ()`)**; System monitoring validity metadata **(`def expire_documents ()`)**. Hard gate and Soft gate in hybrid retrieval: **`def search ()`**; Metadata filtering to narrow retrieval scope: **`def _apply_filters()`**.
        
      * **main.py:** _SAP Document processing module through documents._ Expanding query **(`class InternalQueryExpander`)**; Updating metadata **(`def update_document_metadata ()`)**; System monitoring document validity and sending notification email to decidated persopn **(`def enforce_document_validity ()`**, Constraining the LLM through prompting to avoid hallucination answers **(`def execute ()`, `def execute_stream ()`).**
    
## Frontend

* **va_app.py:** A Streamlit frontend interface for operations including chatting **(`def sap_docs_test_page()`)**, submitting feedback **(`def _submit_feedback()`)**, document management **(`def manage_sap_documents()`)**, and Metadata filtering **`def sap_docs_test_page()`**.
