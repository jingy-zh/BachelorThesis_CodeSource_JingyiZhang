...
* **Service Layer** The core functionality logic.
   ...
   * **sap_documents** Core functionality in sap document management, retrieval, and validity monitoring.
      ...
      * **document_search.py:** **_Managing document embeddings in the vector database._** Inculding creating document, **hybrid retrieving in the database (`def search ()`)**, removing document from the database, **updating metadata fields without re-ingesting embeddings (`def update_document_metadata ()`)**, **system's automated monitoring document validity, and setting status of expired document to 'deprectaed' (`def expire_documents ()`)**. **Hard gate and softe gate** in hybrid retrieval are defined in the **`def search ()`** function. **Metadata filtering to narrow retrieval scope** is definded in **`def _apply_filters()`** 
        
      * **main.py:** **_SAP Document processing module through documents._** **Expanding query (`class InternalQueryExpander`)**; uploading, listing, and deleting document; **updating metadata (`def update_document_metadata ()`)**; **monitoring document validity and sending notification email to decidated persopn (`def enforce_document_validity ()`)**, **constraining the LLM through prompting to avoid hallucination answers (`def execute ()`, `def execute_stream ()`)**.
